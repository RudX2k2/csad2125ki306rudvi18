...
# Define variables
PROJECT_NAME="RPS_Controller"
PROJECT_VERSION="1.0"
PROJECT_DIR="RPS_Controller"
INPUT_DIRS="../$PROJECT_DIR"
OUTPUT_DIR="../docs/controller"
...
# Enable preprocessing and macro expansion
...
sed -i "s|^FILE_PATTERNS.*|FILE_PATTERNS          = *.c *.h *.cpp *.hpp|" "$DOXYFILE_PATH"

# Generate documentation
echo "Generating Doxygen documentation for $PROJECT_NAME..."
...
if [[ -f "$INDEX_FILE" ]]; then
    ...
    echo "Documentation generation failed for $PROJECT_NAME. Check the Doxyfile for errors."
fi
